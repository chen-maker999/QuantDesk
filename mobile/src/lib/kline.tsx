...
export function CandlestickChart({ bars, height = 320, maPeriods = [5, 10, 20] }: { bars: MarketBar[]; height?: number; maPeriods?: number[] }) {
  ...
  const handleMove = (e: React.PointerEvent<SVGSVGElement>) => setCross(hitIndex(e, n, padL, padR));
  ...
  return (
    <div ref={wrapRef} className="kline-wrap" style={{ height }}>
      {width > 0 && (
        <svg width={width} height={height} onPointerMove={handleMove} onPointerDown={handleMove} onPointerLeave={() => setCross(null)} onPointerUp={() => setCross(null)} onPointerCancel={() => setCross(null)}>
