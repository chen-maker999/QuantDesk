...
function DataPage({status,onImported,onProviderKey,onTushareKey,notify}:{status:WorkspaceStatus;onImported:(s:WorkspaceStatus)=>void;onProviderKey:()=>void;onTushareKey:()=>void;notify:Notify}){
  ...
  const importFile=async(file:File)=>{setBusy(true);try{const text=await file.text();const lines=text.trim().split(/\r?\n/);const headers=lines.shift()?.split(",").map(x=>x.trim().toLowerCase())||[];const si=headers.indexOf("symbol"),di=headers.indexOf("date"),ci=headers.indexOf("close");if(si<0||di<0||ci<0)throw new Error("CSV 必须包含 symbol、date、close 列");const rows=lines.filter(Boolean).map(line=>{const cells=line.split(",");return{symbol:cells[si].trim(),date:cells[di].trim(),close:Number(cells[ci])}}).filter(row=>row.symbol&&row.date&&Number.isFinite(row.close)&&row.close>0);if(!rows.length)throw new Error("文件中没有有效价格记录");const next=await importMarketRows(rows);onImported(next);notify(`已导入 ${rows.length.toLocaleString()} 行真实价格数据`)}catch(e){notify(e instanceof Error?e.message:"导入失败","error")}finally{setBusy(false);if(input.current)input.current.value=""}};
  ...
  return <div className="page-body v3-page"><input ref={input} hidden type="file" accept=".csv,text/csv" onChange={e=>{const file=e.target.files?.[0];if(file)void importFile(file)}}/>
    ...
    <div className="source-real card"><span><Database/></span><div><h2>CSV 市场数据</h2><p>也可导入 symbol、date、close。数据只保存在本机。</p></div><MiniBadge tone={status.market_rows?"green":"gray"}>{status.market_rows?"已有数据":"未导入"}</MiniBadge><button className="secondary-btn" disabled={busy} onClick={()=>input.current?.click()}>{busy?<RefreshCw className="spin"/>:<FileUp/>}{status.market_rows?"追加 CSV":"选择 CSV"}</button></div>{status.market_rows>0&&<div className="real-data-summary"><div className="card"><small>价格记录</small><strong>{status.market_rows.toLocaleString()}</strong></div><div className="card"><small>标的数量</small><strong>{status.market_symbols}</strong></div><div className="card"><small>最新日期</small><strong>{status.market_latest}</strong></div></div>}<div className="format-help card"><h3>CSV 格式</h3><pre>symbol,date,close{"\n"}000001.SZ,2026-08-21,12.35</pre><p>不会自动生成、补齐或插值缺失记录。</p></div></div>;
}
