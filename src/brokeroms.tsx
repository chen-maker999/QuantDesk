...
import {
  ...
  getBrokerOrders, getBrokerPositions, hasBrokerCredentials, listBrokers, lookupIbkrContracts, placeBrokerOrder,
  saveBrokerCredentials, type BrokerCredentials, type BrokerId, type BrokerOrder, type BrokerPosition, type BrokerStatus,
} from "./lib/brokers";
...
const defaults: Record<BrokerId, Draft> = {
  alpaca: { api_key: "", api_secret: "", gateway_url: "", account_id: "", trading_mode: "paper", max_order_notional: 1000, max_open_orders: 10 },
  ibkr: { api_key: "", api_secret: "", gateway_url: "https://localhost:5000/v1/api", account_id: "", trading_mode: "paper", max_order_notional: 1000, max_open_orders: 10 },
};
...
export function BrokerOmsPage({ notify }: { notify: Notify }) {
  ...
  const save = async () => {
    ...
    try {
      const payload: BrokerCredentials = active === "alpaca"
        ? { api_key: currentDraft.api_key.trim(), api_secret: currentDraft.api_secret.trim(), trading_mode: currentDraft.trading_mode, max_order_notional: currentDraft.max_order_notional, max_open_orders: currentDraft.max_open_orders }
        : { gateway_url: currentDraft.gateway_url.trim(), account_id: currentDraft.account_id.trim(), trading_mode: currentDraft.trading_mode, max_order_notional: currentDraft.max_order_notional, max_open_orders: currentDraft.max_open_orders };
  ...
  return <div className="page-body broker-oms">
    <div className="broker-oms-head"><div><h3>实盘 OMS</h3><p>券商下单与 Agent、模拟盘完全隔离。凭据仅保存在 Windows Credential Manager；真实模式必须每次会话重新解锁。</p></div><button className="secondary-btn" disabled={busy} onClick={() => void refreshTrading()}><RefreshCw size={13} />同步账户</button></div>
    ...
    <div className="broker-grid">
      <section className="card broker-config"><h3><KeyRound size={15} />连接配置</h3>
        ...
        <div className="broker-inline broker-inline-single"><label>最大挂单数<input type="number" min="1" max="200" value={currentDraft.max_open_orders} onChange={e => update("max_open_orders", Number(e.target.value))} /></label></div>
